...
class Prior(object):
    ...
    def __init__(self, pcat=None, scat=None, rndcat=True, radius=5*u.arcsec,
                 magmin='auto', magmax='auto', magbinsize='auto',
                 match_mags=None, prior_dict=None):
        """
    ...
    def _from_catalogues(self, pcat, scat, match_mags, rndcat, 
        ...
        self.prior_dict = self._calc_prior_dict(pcat, scat, radius, match_mags,
                                                magmin, magmax, magbinsize)


    @classmethod
    def from_nway_hists(cls, catname, magnames, path='.'):
        """
        Create a ``Prior`` object using nway histogram files.
        """
        prior_dict = {}
        for mag in magnames:
            filename = '{}_{}_fit.txt'.format(catname, mag)
            filename = os.path.join(path, filename)
            prior_dict[mag] = cls._from_nway_maghist(filename)
        
        return cls(prior_dict=prior_dict)

    
    def get_magnames(self):
        """
        Return a list with the magnitude names stored in the ``Prior`` object.
        """
        return [key for key in self.prior_dict]
    ...
    def to_nway_hists(self, output_path=None):
        ...
        """
        nhists = []
        for magcol in self.get_magnames():
    ...
    def interp(self, mags, magcol):
        ...
        itp = interp1d(bins, prior['good'], kind='nearest',
                       fill_value=0, bounds_error=False)
        
        pvals = itp(mags)

#        mask_inside_magrange = np.logical_and(mags >= prior['bins'][0],
#                                              mags < prior['bins'][-1])
#
#        idx_bins = np.digitize(mags[mask_inside_magrange], prior['bins'])
#
#        pvals = np.zeros_like(mags)
#        pvals[mask_inside_magrange] = prior['good'][idx_bins]
    ...
    def qcap(self, magcol):
        ...
        return np.sum(prior['good'] * np.diff(prior['bins']))
    ...
    def to_table(self):
        ...
        """
        magnames = self.get_magnames()

        # TODO: how to build this table when magbinsize is 'auto'
        priors_table = Table()
        priors_table['MAG'] = self.bins_midvals(magnames[0])
        for mag in magnames:
            priors_table['PRIOR_' + mag] = self.prior_dict[mag]['good']
    ...
    def _calc_prior_dict(self, pcat, scat, radius, match_mags, magmin,
                         magmax, magbinsize, mask_radius=1*u.arcmin):
        ...
        for magcol in match_mags.colnames:
            good_mags = match_mags[magcol]
            field_mags = field_cat.mags[magcol]
            prior_dict[magcol] = self._mag_hist(
                                        len(pcat), good_mags, field_mags,
                                        renorm_factor, magmin, magmax, magbinsize)
        return prior_dict
    ...
    def _mag_hist(self, pcat_nsources, good_mags, field_mags,
                  renorm_factor, magmin, magmax, magbinsize):

        if magmin == 'auto':
            magmin = np.nanmin(good_mags)
            
        if magmax == 'auto':
            magmax = np.nanmax(good_mags)

        bins, magrange = _define_magbins(magmin, magmax, magbinsize)
        good_counts, bins = np.histogram(good_mags, range=magrange, bins=bins)
        ...
        good_prior = convolve(good_prior, [0.25, 0.5, 0.25])                
...
#            good_prior = 0.999 * good_prior / test
        ...
        maghist = {}
        maghist['bins'] = bins
        maghist['good'] = good_prior / pcat_nsources / magbinsize
        maghist['field'] = 1.0*field_counts / len(field_mags) / magbinsize
...
class BKGpdf(object):
    ...
    def interp(self, mags, magcol):
        ...
        itp = interp1d(bins, pdf, kind='nearest', 
    ...
    def _calc_pdf(self, cat, magmin, magmax, magbinsize):
        ...
        for magcol in mags.colnames:
            pdf_dict[magcol] = self._mag_hist(mags[magcol], area, 
                                              magmin, magmax, magbinsize)
        return pdf_dict
