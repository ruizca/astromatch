...
class SkyCoordErr(object):
    ...
    def __getitem__(self, key):
        return SkyCoordErr(self.components[key], errtype=self.errtype, check=False)
    ...
    def add_syserr(self, errsys):
        """
        Add systematic to the error components. Only works for circular errors.
        """
        if self.errtype == 'circle':
            self.components.columns[0] = np.sqrt(errsys**2 +
                                                 self.components.columns[0]**2)
        else:
            raise NotImplementedError
    ...
    def randomise(self, r_min=20*u.arcsec, r_max=120*u.arcsec,
        ...
        if numrepeat == 1:
            ...
            with tempfile.NamedTemporaryFile() as input_file:
                ...
                rnd_cat = Catalogue(rnd_cat_data, area=area,
                                    poserr_cols=poserr_cols, name=self.name)
                
        else:
            # Use seed != None only for testing, to obtain the same random catalogue        
            ra, dec = self._random_coords(0*u.deg, 360*u.deg, r_min, r_max, numrepeat, seed)
            ...
            with warnings.catch_warnings():
                ...
                rnd_cat = Catalogue(rnd_cat_data, area=area, poserr_cols=None,
                                    name=self.name)
    ...
    **kwargs :
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile() as xms_file:
            _make_xms_file(xms_file.name, prefix=cat_prefix, 
                           fmt=cat_fmt, seed=seed, **kwargs)
        ...
        for tag in ascii_uppercase[:kwargs['nTab']]:
            ...
            cat = Catalogue(cat_file, area=area, id_col='id',
                            coord_cols=['posRA', 'posDec'],
                            poserr_cols=['ePosA', 'ePosB', 'ePosPA'], 
                            poserr_type='ellipse', name=tag + 'mock')
...
def _make_xms_file(filename, prefix='tmp_mock', fmt='fits', seed=None, **kwargs):
    ...
    args_str =  ' '.join('{}={}'.format(key, value) 
